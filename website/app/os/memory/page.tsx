...
import { mockMemory, mockFactories, formatRelativeTime } from '@/lib/mock'
import type { MemoryEntry, MemoryTag } from '@/types'
...
function MemoryDetail({ entry, onClose }: { entry: MemoryEntry; onClose: () => void }) {
  const factory = mockFactories.find(f => f.id === entry.factoryId)
  return (
    <motion.div
...
function MemoryRow({ entry, onClick }: { entry: MemoryEntry; onClick: () => void }) {
  const factory = mockFactories.find(f => f.id === entry.factoryId)
  return (
    <motion.button
      onClick={onClick}
...
export default function MemoryPage() {
  ...
  }, [search, tagFilter, factoryFilter])

  const totalSize = mockMemory.reduce((s, m) => s + m.size, 0)
    ...
    <div className="space-y-5 max-w-5xl">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-slate-600 mt-0.5">
            {mockMemory.length} 件 · 合計 {totalSize.toLocaleString()} chars
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1">
        {Object.entries(tagConfig).map(([tag, cfg]) => {
          const count = mockMemory.filter(m => m.tags.includes(tag as MemoryTag)).length
          return (
            <button
              key={tag}
              ...
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 border text-[11px] font-mono whitespace-nowrap transition-colors ${
                tagFilter === tag ? cfg.color : 'border-white/[0.06] bg-white/[0.02] text-slate-600 hover:text-slate-400'
              }`}
            >
              {cfg.label}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          ...
          {mockFactories.filter(f => f.memoryItems > 0).map(f => (
      ...
      <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
        {filtered.length === 0 ? (
          ...
          filtered.map(entry => (
            <MemoryRow key={entry.id} entry={entry} onClick={() => setSelected(entry)} />
          ))
        )}
      </div>
      ...
      <AnimatePresence>
        {selected && (
          <>
            ...
            />
            <MemoryDetail entry={selected} onClose={() => setSelected(null)} />
          </>
        )}
      </AnimatePresence>
    </div>
